...
import { 
  ...
  HelpCircle, CheckCircle, AlertCircle, Trash2
} from "lucide-react";
...
export default function StorePage() {
  ...
  const handleDeleteProduct = async (productId, productName) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(`Successfully deleted ${productName}!`);
        fetchProducts();
      } else {
        const data = await res.json();
        showToast(data.message || `Failed to delete ${productName}`, "error");
      }
    } catch (err) {
      console.error(err);
      showToast("Connection failure during product deletion", "error");
    }
  };
                    ...
                    <div>
                      ...
                      <div className="relative mb-4">
                        <Link href={`/store/product/${p._id || p.id}`} className="block">
                          <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-gray-50 dark:bg-neutral-950 border border-gray-100 dark:border-neutral-800/80 cursor-pointer">
                            <Image
                        ...
                        {currentUser?.role === "admin" && (
                          <button
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              handleDeleteProduct(p._id || p.id, p.name);
                            }}
                            className="absolute top-3 right-3 p-2 rounded-xl bg-red-600/90 hover:bg-red-600 text-white shadow-md hover:scale-105 active:scale-95 transition-all duration-200 z-20 cursor-pointer border border-red-500/20"
